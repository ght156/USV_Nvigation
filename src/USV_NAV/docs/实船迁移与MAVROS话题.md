...
| 项 | 说明 |
|----|------|
| **执行层** | 真船无 **`/usv/thrusters/...`**。需 **停用或替换 `converter.py`**：常见通路为 MAVROS **`/mavros/setpoint_velocity/cmd_vel(_unstamped)`**（Offboard）、**`/mavros/rc/override`**、**`/mavros/actuator_control`** 等，与队里 PX4 **机架/混控**一致。**本仓库实船默认**：**`nav2_cmd_vel_to_mavros`**（**`/cmd_vel`→`/mavros/setpoint_velocity/cmd_vel_unstamped`**）。仿真与真机 **不可混用同一套增益**。 |
