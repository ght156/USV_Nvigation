# Nav2 参数详解与调参指南（USV 仿真）

本文逐一解释 `workspace_nav/config/nav2_params.yaml` 中每个参数的含义、原理、对 USV 运动的影响，以及遇到问题时的调参方向。

**适用场景**：Gazebo 仿真，双体 USV，差动推进，RegulatedPurePursuitController + SmacPlanner2D。
...
| `odom_topic` | 里程计话题 | 行为树用这个判断机器人是否在移动、是否到达目标 | 必须与 EKF 输出一致，本项目为 `/odometry/filtered` |
...
**对 USV 的影响**：Lookahead 太小 → 船盯着脚底下频繁修舵 → S 线。Lookahead 太大 → 走捷径切弯 → 可能撞障碍物。
...
| `min_lookahead_dist` | 速度趋近 0 时的最小前瞻距离 | **对船太小（如 1m）**：低速时仍盯近点，船转不过来就画圈 |
| `max_lookahead_dist` | 高速时的最大前瞻距离 | 限制 carpot 不会太远，太远会抄近道切弯 |
...
| 现象 | 诊断 | 调参方向 |
|------|------|---------|
| 蛇形/画圈 | carrot 太近，船追着脚下频繁修舵 | **增大 min_lookahead_dist** |
| 转弯切角严重 | carrot 太远，船抄近道 | **减小 max_lookahead_dist** |
| 路径跟踪太迟钝 | carrot 太远，反应慢 | **减小 lookahead_time** |
...
velocity_smoother:
  ros__parameters:
    ...
    feedback: "CLOSED_LOOP"          # 闭环模式
    max_velocity: [2.0, 0.0, 2.0]    # [max_linear_x, max_linear_y, max_angular_z]
    min_velocity: [0.0, 0.0, -2.0]   # [min_linear_x, min_linear_y, min_angular_z]
    max_accel: [2.0, 0.0, 1.0]       # 最大加速度 [x, y, yaw]
    max_decel: [-2.0, 0.0, -2.0]     # 最大减速度
    odom_topic: "odometry/filtered"
    odom_duration: 0.1
...
| **S 线（左右摆）** | lookahead 太小 + 速度太快 + 船惯性大 | `desired_linear_vel` ↓、`min_lookahead_dist` ↑、`max_angular_accel` ↓ |
...
- 改 RPP 速度参数 → 同步检查 converter 的推力映射
