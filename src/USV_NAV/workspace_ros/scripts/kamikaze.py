...
IMAGE_TOPIC = "/usv/sensors/camera/image"
...
class CameraSubscriber(Node):
    ...
    def _load_model(self):
        try:
            ...
            for _ in range(8):
                ...
                candidate2 = os.path.join(cur, "USV_NAV", "workspace_ros", "YOLOv11", "YOLOv11.pt")
                ...
                if parent == cur:
                    break
                cur = parent
            fallback = os.path.join(os.getcwd(), "src", "USV_NAV", "workspace_ros", "YOLOv11", "YOLOv11.pt")
    ...
    def _resolve_target_json_path(self) -> str:
        ...
        for _ in range(8):
            ...
            if os.path.isfile(candidate):
                return candidate
            candidate2 = os.path.join(cur, "USV_NAV", "workspace_nav", "json", TARGET_JSON_FILENAME)
            if os.path.isfile(candidate2):
                return candidate2
            candidate3 = os.path.join(cur, "src", "USV_NAV", "workspace_nav", "json", TARGET_JSON_FILENAME)
            ...
            if parent == cur:
                break
            cur = parent
        cwd_candidate = os.path.join(os.getcwd(), "src", "USV_NAV", "workspace_nav", "json", TARGET_JSON_FILENAME)
        if os.path.isfile(cwd_candidate):
            return cwd_candidate
        alt_cwd = os.path.join(os.getcwd(), "USV_NAV", "workspace_nav", "json", TARGET_JSON_FILENAME)
