...
> **version: 5.0** | 供**第三方上层**（任务编排、竞赛主控等）对接本导航栈  
> 定位模块对接（SLAM 等）见 §2 及 [`src/USV_NAV/docs/SLAM_INTEGRATION.md`](../src/USV_NAV/docs/SLAM_INTEGRATION.md)。
> **实船副本**：与仿真仓 `wuxihik_navigation/docs/nav_task_interface.md` 同步；船端 launch/YAML 见 [`../src/USV_NAV/docs/mission_bridge与上层对接.md`](../src/USV_NAV/docs/mission_bridge与上层对接.md)。  
...
## 2. 定位数据来源（SLAM / 里程计 → 导航）

> **version: 5.0** | 本节为**内部定位模块**对接说明，供 SLAM 组员参考；上层任务系统无需关心。  
> 导航栈消费的定位数据 = **一个 `Odometry` 话题 + 一条 TF 链 `map→odom→base_link`**。  
> 当前默认来源为 MAVROS（PX4 EKF），后续切换为 SLAM 输出。

### 2.1 数据流

```
SLAM / 定位模块                       Nav2 + mission 栈
──────────────────                    ──────────────────
/slam/localization (Odometry)  ──►  bt_navigator, controller_server,
                                   velocity_smoother, mission_bridge,
                                   nav_status_aggregator

TF: map → odom                 ──►  global_costmap (global_frame: map)
TF: odom → base_link           ──►  local_costmap (global_frame: odom)
                                   behavior_server, planner_server
```

### 2.2 话题接口定义

#### 2.2.1 `/slam/localization` — 定位位姿（SLAM → 导航）

| 项目 | 说明 |
|------|------|
| 类型 | `nav_msgs/Odometry` |
| QoS | BEST_EFFORT, depth≥10 |
| 频率 | ≥ 20 Hz（低于此频率会被判 DEGRADED） |
| `header.frame_id` | `"odom"` |
| `child_frame_id` | `"base_link"` |

**必须填写的字段**（缺字段会导致定位判 LOST 或速度/航向不可用）：

| 字段路径 | 类型 | 用途 | 判据 |
|----------|------|------|------|
| `pose.pose.position.x` | float64 | 当前位置 x (m) | 航点接近判断、状态上报 |
| `pose.pose.position.y` | float64 | 当前位置 y (m) | 同上 |
| `pose.pose.orientation` | Quaternion | 当前航向（四元数→yaw） | 航向显示、航点跟踪 |
| `twist.twist.linear.x` | float64 | 前向速度 (m/s) | 速度显示、stuck 检测 |
| `twist.twist.angular.z` | float64 | 转向角速度 (rad/s) | 角速度显示 |
| `pose.covariance[0]` | float64 | 位置 x 方差 (m²) | > 10 → LOST, > 1 → DEGRADED |
| `pose.covariance[7]` | float64 | 位置 y 方差 (m²) | 同上 |
| `pose.covariance[14]` | float64 | 位置 z 方差 (m²) | 同上（水面可填小值） |
| `pose.covariance[21]` | float64 | 姿态 roll 方差 (rad²) | 同上 |
| `pose.covariance[28]` | float64 | 姿态 pitch 方差 (rad²) | 同上 |
| `pose.covariance[35]` | float64 | 姿态 yaw 方差 (rad²) | 同上 |
| `header.stamp` | builtin_interfaces/Time | 时间戳 | 超时 2s → LOST |

**`pose.pose.position.z`** 水面导航未使用（填充 0 即可）。  
**`twist.twist.linear.y`** 侧向速度当前未使用（填充 0 即可）。

#### 2.2.2 TF 广播（SLAM → 导航）

| 变换 | 父帧 | 子帧 | 由谁发布 | 说明 |
|------|------|------|----------|------|
| `map → odom` | `map` | `odom` | **SLAM 节点** | SLAM 的全局位姿修正（漂移消除后的偏移量）；≥20 Hz |
| `odom → base_link` | `odom` | `base_link` | **SLAM 节点**（或 Odometry 消息自带） | 里程计局部位姿；替代当前 MAVROS 的发布 |

**为什么分两层？**  
- `odom → base_link`：连续、平滑的局部里程计（高频，会有累积漂移）  
- `map → odom`：SLAM 通过回环/特征匹配消除漂移后的全局修正（离散跳变）  
- Nav2 通过 `map → odom → base_link` 链得到全局无漂移位姿

**SLAM 直接输出 `map → base_link`** 也可以工作，但需要将 Nav2 local_costmap 的 `global_frame` 改为 `map`，且失去里程计与全局位姿的解耦能力。推荐使用标准两层结构。

### 2.3 坐标系对齐（SLAM 与导航栅格地图的关系）

**核心问题**：SLAM 启动时的自身原点 ≠ 导航 OccupancyGrid（pgm）的原点。  
**解决方式**（三选一，推荐方式 A）：

#### 方式 A：初始位姿对齐（推荐）

SLAM 接收 `/initialpose`（`geometry_msgs/PoseWithCovarianceStamped`, `frame_id: map`），将自身坐标系与导航 map 系对齐。

```
导航侧 ──► /initialpose { pose: (x,y,yaw in map), covariance: ... }
SLAM侧 ──► 收到后重置自身原点，使当前位置 = 指定 map 坐标
```

- 初始位姿可由 GNSS 提供（通过 `map.yaml` 中的 `ref_gnss_*` 锚点 + 当前 GNSS 计算）
- 或由 `gnss_odom_map_tf` 节点在启动时计算并发送一次
- **SLAM 不需要收到导航栅格地图**，只需要知道初始时自己在 map 系下的绝对坐标

#### 方式 B：导航地图共享

导航将 OccupancyGrid 发布给 SLAM，SLAM 在其中做基于地图的定位。

| 项目 | 说明 |
|------|------|
| 话题 | `/map`（`nav_msgs/OccupancyGrid`，已由 `map_server` 发布） |
| QoS | RELIABLE + TRANSIENT_LOCAL |
| 包含信息 | 栅格数据 + `header.frame_id: map` + 分辨率 + 原点（地图左下角在 map 系坐标） |

**SLAM 侧需做**：订阅 `/map`，在栅格地图中进行扫描匹配/重定位，输出 map 系位姿。  
**适用场景**：SLAM 有 LiDAR/视觉特征图且需要与导航地图对齐。

#### 方式 C：统一 GNSS 锚点（当前方案变体）

SLAM 和导航地图共用同一个 GNSS 锚点（`map.yaml` 中的 `ref_gnss_*`），各自独立计算 map 系坐标：

```
map 系原点 = 锚点 GNSS 坐标（经度, 纬度）
当前 map 坐标 = ENU(当前 GNSS - 锚点 GNSS)
```

SLAM 融合 GNSS 初始化即可输出 map 系位姿，无需 `/initialpose`。

### 2.4 导航 → SLAM 的可选输出

| 接口 | 类型 | 是否必须 | 用途 |
|------|------|----------|------|
| `/map` | `nav_msgs/OccupancyGrid` | 否 | 方式 B：SLAM 在导航地图中定位 |
| `/initialpose` | `geometry_msgs/PoseWithCovarianceStamped` | 否 | 方式 A：SLAM 初始化坐标系对齐 |
| map 锚点参数 | ros2 param（`map_yaml_path`） | 否 | 方式 C：SLAM 获取 ENU 投影参数 |

### 2.5 切换为 SLAM 需修改的配置

从 MAVROS 定位切换到 SLAM 定位时，需统一修改以下 topic 和 frame 引用：

| 配置文件 | 参数路径 | 当前值 | 改为 SLAM 后 |
|----------|----------|--------|-------------|
...
导航侧需要从 SLAM 获取以下信息用于状态监控：

| 参数 / 能力 | 说明 |
|-------------|------|
| 定位坐标系是否与导航 map 系对齐 | 是 / 否；若否，说明对齐方式 |
| 输出频率 | ≥ 20 Hz 为健康，< 20 Hz 触发 DEGRADED |
| 协方差填充策略 | 动态（随定位质量变化）或静态（固定值）；静态值需 < 1.0 避免误判 DEGRADED |
| 是否需初始位姿 | 是 / 否；若需，接受 `/initialpose` 还是其他方式 |
...
### 6.1 Service 接口（上层 → 导航，推荐）

#### 6.1.1 `/mission_bridge/send_waypoints` — 下发航线
...
#### 6.1.2 `/mission_bridge/set_pause` — 暂停/继续
...
#### 6.1.3 `/mission_bridge/emergency_stop` — 急停
...
#### 6.1.4 `/mission_bridge/cancel_mission` — 取消 / 退出急停
...
### 6.2 地面站 GCS 专用 Topic（保留，与上层 Service 并行）
...
#### 6.2.3 `/gcs_mission/cancel` — 取消 / 清除急停（GCS）
...
#### 6.2.4 `/gcs_mission/start` — 开始（仅 `start_pulse`）
...
### 6.5 `/mission_bridge/state` — 回退（可选）
...
### 7.4 Nav2 FollowWaypoints 错误 → 上层可见信号的链路
...
### 9.3 Topic 方式 start_pulse（发航点 + 开始）
