...
检查项（check_rate 周期执行）：
  ...
  3. odom 推算超时（正常窗口 odom_prediction_timeout；
     ALIGN_ENTRY 放宽到 align_odom_prediction_timeout——必须大于 FSM
     align_tag_loss_grace_sec，否则 safety 抢跑 ODOM_TIMEOUT 使 ALIGN
     的 REACQUIRE 重捕获路径永远走不到；
     ABORT_EXIT/UNDOCK 期间放宽到 abort_exit_odom_timeout；
     ACQUIRE_TAG/REACQUIRE_TAG 搜索状态豁免，由 FSM 超时兜底）
                                                -> abort_request("ODOM_TIMEOUT")
     撤离通道（EXIT_STATES）位姿 INVALID 持续超 exit_pose_invalid_warn_sec
                                                -> abort_request("EXIT_POSE_LOST")
                                                   （提示性告警：FSM 仅记录不改状态，
                                                   驶出仍由 FSM 超时兜底）
...
# 完全静默状态
# WAIT_DOCK_OPEN/WAIT_DOCK_RELEASE（夹爪模式）：船静止停车等待船坞夹爪动作，
# 坞内夹爪扣住时位姿可能 INVALID，若按活动状态查 odom 推算会在 ControlDO
# 电机动作（约3s）完成前就误触发 ODOM_TIMEOUT -> FAILED（2026-09-03 评审发现）
QUIET_STATES = (
    "IDLE",
    "DOCKED",
    "FAILED",
...
class DockingSafety(Node):
    def __init__(self):
        super().__init__("docking_safety")
        ...
        # ALIGN_ENTRY 独立推算窗口：须 > FSM align_tag_loss_grace_sec（默认 5s，
        # 取 +2s 余量），否则 safety 以 3s+0.5s 防抖抢跑 ODOM_TIMEOUT，
        # ALIGN 的 REACQUIRE 重捕获路径永远走不到；APPROACH 保持 3s 不动
        # （与 FSM approach 宽限 3s 的 0.5s 竞速差是刻意设计）
        self.declare_parameter("align_odom_prediction_timeout", 7.0)
        self.declare_parameter("abort_exit_odom_timeout", 60.0)
        # 撤离通道位姿 INVALID 持续告警：只发提示信号 EXIT_POSE_LOST
        #（FSM 仅记录不改状态），驶出仍由 FSM 超时兜底
        self.declare_parameter("exit_pose_invalid_warn_sec", 5.0)
        ...
        # 撤离通道位姿 INVALID 起始时刻（None=未处于丢失中）
        self._exit_invalid_since = None

        rate = float(self.get_parameter("check_rate").value)
        self.create_timer(1.0 / rate, self._check)

        self.get_logger().info("docking_safety 已启动（6 项检查）")
    ...
    def _check(self):
        ...
        if state != self._last_state:
            ...
            self._exit_invalid_since = None  # 跨状态重新计时撤离位姿丢失
        ...
        if (
            ...
            elif state == "ALIGN_ENTRY":
                # 见 align_odom_prediction_timeout 参数注释：不得抢跑 FSM
                # align_tag_loss_grace_sec 宽限
                odom_limit = float(p("align_odom_prediction_timeout").value)
            else:
            ...
            if self._pose_source == SRC_INVALID or (
                ...
                if self._pose_source == SRC_INVALID and state in EXIT_STATES:
                    # 撤离中位姿丢失：不中止（FSM 用超时兜底驶出），
                    # 但持续超阈值要发提示性告警，避免"静默盲开"
                    if self._exit_invalid_since is None:
                        self._exit_invalid_since = now
                    elif (now - self._exit_invalid_since).nanoseconds * 1e-9 > float(
                        p("exit_pose_invalid_warn_sec").value
                    ):
                        abort_reason = "EXIT_POSE_LOST"
                else:
                    self._odom_count += 1
