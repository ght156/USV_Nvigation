...
# ----------------------------------------------------------------------------------------------- #
#  无头（server-only）版仿真启动：与 simulation.launch.py 相同，但 gz sim 以 -s -r 运行，
#  不开 GUI，适合远程/自动化测试。相机传感器在无头模式下仍渲染（EGL/软件渲染）。
#  用法：ros2 launch workspace_gz simulation_headless.launch.py
# ----------------------------------------------------------------------------------------------- #
