# USV_NAV 项目架构与 Nav2 说明

本文档面向复盘与二次开发，梳理 RoboBoat 仿真栈的**包结构、数据流、定位、执行器**以及 **Navigation2 的规划–避障–控制**链路。对应代码路径以工作空间根目录 `usv_nav_ws/` 为参照。
...
| 包名 | 角色 |
|------|------|
| `workspace_gz` | Gazebo Garden 仿真：`world`、`usv` 模型、`xacro`、`ros_gz` 桥（时钟、传感器、推进器指令） |
...
```
  ...
  │  converter          │
  └──────────┬──────────┘
             │ /usv/thrusters/left|right/thrust
             ▼
  ┌─────────────────────┐
...
3. 本项目的 **`converter`** 订阅 **`/cmd_vel_nav`**，把角/线速度换算为左右 **`/usv/thrusters/.../thrust`**。  
...
```
Nav2 controller_server → /cmd_vel_nav → converter.py → /usv/thrusters/.../thrust → ros_gz_bridge → Gazebo
```
...
- **`simulation.launch.py`**：设置 `GZ_SIM_*` / `IGN_GAZEBO_*` 资源路径，启动 `gz sim <world.sdf>`，`robot_state_publisher` + `joint_state_publisher`，`ros_gz_sim create` 生成 `usv`。
- **`parameter_bridge`**：将仿真侧话题桥到 ROS 2，主要包括：
  - `/clock`
  - 左右推力：`.../cmd_thrust` ↔ `/usv/thrusters/left/thrust`、`/usv/thrusters/right/thrust`
...
| MAVROS→`/usv` | 模式 A (EKF) 下 **`imu_covariance_repub`** / **`gps_covariance_repub`** 可直接订阅 MAVROS 话题（通过 **`imu_src`** / **`gps_src`** 参数），不再需要中继节点。模式 B (mavros_odom) 不经过此链。|
...
- **发布**：`/usv/thrusters/left/thrust`、`/usv/thrusters/right/thrust`（`std_msgs/Float64`），经 Gazebo 桥驱动推进器插件。
...
- **效果**：Gazebo **`gz-sim-navsat-system`** 与船上 NavSat 传感器按该原点把 **机体在世界中的位姿 → 经纬度**。船在 **`simulation.launch.py` 中 spawn 在原点附近**（默认 `x=y=z=0`）时，**话题里报告的经纬度就会在「新原点」附近**。  
  因此：**改原点 → 仿真 `/usv/sensors/gps/navsat` 的读数会变**。

### 11.2 地面站界面上的「船位经纬」从哪来？改仿真原点会反映在 UI 上吗？

上游 **GROUND CONTROL STATION**（独立仓库，Electron + 后端 ROS 订阅）里，后端订阅 **`/gps/fixed_cov`** 等话题，再通过 HTTP 给前端 Cesium。**本栈中** **`gps_covariance_repub`** 将仿真 **`/usv/sensors/gps/navsat`** 重发布后得到 **`/gps/fixed_cov`**。
...
| **全局/局部动态障碍层** | **仿真**：**`ObstacleLayer`** + **`/usv/sensors/lidar/scan`**（**`LaserScan`**）。**实船 `nav2_params_real_mavros`**：**`VoxelLayer`** + **`PointCloud2`**（默认 **`/livox/lidar`**），再经 **2D 膨胀** 推开路径。仿真里若没有有效障碍物，表现会接近「空地绕路」。 |
...
若在主目录下复制本工作空间用于二次开发，可使用与 `usv_nav_ws` **同级** 的目录 **`/home/ght/wuxihik_navigation`**（仅拷源码与工具，不含 `build`/`install`/`log` 时更干净）。进入后：
...
若 Nav2 等依赖安装在 **隔离工作空间**，需先 `source` 该工作空间的 `install/setup.bash` 再 `colcon build`。本文档在衍生树中路径仍为 **`src/USV_NAV/docs/PROJECT_ARCHITECTURE_AND_NAV2.md`**（若复制时未改仓库目录名）。
