...
──────────────────────              ────────
/waypoint（可配）                 ──►  WGS84 JSON 航线（debounce 后自动开始）
...
> **本节仅适用于地面站 Topic `/waypoint`**，与上层 Service `send_waypoints` 无关（Service **调用即开始**）。

GCS `/waypoint` 使用固定 **debounce** 语义：收到航线后，**静默约 0.45s**（无新 `/waypoint`）即**自动开始**，**不需要**再发「开始导航」或 `/gcs_mission/start`。

| GCS 行为 | 说明 |
|----------|------|
| 发 `/waypoint` JSON | 写入 debounce 缓冲；静默窗口结束后执行**最后一次**航线 |
| burst 重复发包 | 正常（如 Run Mission 连发 3 次）；debounce 会去重，只跑最终一版 |
| Run Mission / 换线 | JSON 中带 **`explicit_replan: true`**（运行中换线、Cancel 后再跑通常需要） |

静默窗口由 launch 参数 **`waypoint_commit_delay_sec`** 控制（默认 **0.45** s）。

> **注意**：以上仅适用于 Topic 接口 `/waypoint`。使用 **Service 接口 `send_waypoints`** 时立即执行，无 debounce。
...
默认话题名（均可通过 launch 参数 `waypoint_topic`、`color_topic`、`mission_cancel_topic` 修改）：

| 话题 | 类型 | 用途 |
|------|------|------|
| `/waypoint` | `std_msgs/String` | 下发 WGS84 航线 JSON（debounce 后自动开始） |
...
|------|------|
| 类型 | `std_msgs/Empty` |
| 作用 | RUNNING/PAUSED：停止任务、清 debounce 缓冲；EMERGENCY：清除急停标志→IDLE；抑制后续被动航点（若 `suppress_passive_waypoints_after_cancel=true`） |
...
| # | 项 |
|---|-----|
| 1 | GCS `/waypoint` 发完约 0.45s 内无新包即自动开走 — **无需**单独「开始」话题 |
...
| 不要 | 应该 |
|------|------|
| 假设必须发「开始导航」才走 | GCS 只发 `/waypoint` 即可（debounce 后自动开始）；Service 调用即开始 |
| 用 `/rosout` 判断任务状态 | `/nav_status` + `/task_event` |
| Cancel 后直接发航点且不带 `explicit_replan` | 带 `explicit_replan: true`；Service 方式自带抢占语义 |
