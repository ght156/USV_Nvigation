...
| **`workspace_nav`** | `nav2.launch.py`、`nav2_params.yaml`、默认 **`config/map_hk.yaml`**、航点 / `mission_bridge` |
| **`apriltag_localization`** | 船坞 AprilTag → `/apriltag_node/dock_pose` |
| **`usv_docking`** | 归港入泊：corridor-gated 倒车，发布 `/cmd_vel_nav`（见 [`README.md`](../../usv_docking/README.md)） |
...
3. **`workspace_nav` `nav2.launch.py`** — Nav2 + RViz（默认 `map:=` 包内 `map_hk.yaml`）  
4. **（可选）** `ros2 run workspace_ros converter` — Nav2 `/cmd_vel_nav` → Gazebo 推力（与实船无关）  
5. **（归港）** `apriltag_localization` + `usv_docking` — Tag 定位 → 倒船入泊（须先 Nav2 到预泊点、deactivate controller）
...
Gazebo ──► IMU/GPS/odom ──► covariance repub ──► EKF + navsat ──► /odometry/filtered
                                                      │
map_server (map_hk.yaml) ◄── Nav2 ◄────────────────────┘  (map frame)
     │
     └── local_costmap ◄── LaserScan /roboboat/sensors/lidar/scan

归港（可选）：
  Gazebo 后向相机 ──► apriltag_localization ──► /apriltag_node/dock_pose ──► usv_docking ──► /cmd_vel_nav ──► converter
  （入泊前 deactivate Nav2 controller_server；仿真须 camera_rear_link → base_link TF；实船 camera_left_link）
```
...
- GCS 发 **`/waypoint`** （见 **`gps_map_conversion.parse_waypoint_message`**：`waypoints`、`mission_id` 与可选 **`explicit_replan` / `from_start_mission` / `command` 等**，用于显式重申航迹）与可选 **`/color_code`**：
  - **`mission_bridge`**（`enable_mission_bridge:=true` 或单独 `mission_bridge.launch.py`）：GCS `/waypoint` **debounce**、`mission_cancel_topic`、**`suppress_passive_waypoints_after_cancel`**、写 **`target_buoy.json`**、逐点 **FollowWaypoints**。
  - **`/mission_bridge/state`**（`std_msgs/String`，2026-05-26 新增）：mission_bridge 在每个任务状态变更时（IDLE → RUNNING → COMPLETED/FAILED → IDLE）发布对应状态值。GCS 后端通过 `ros_subscriber.py` 订阅此话题，合并到 `/api/mission_status` 的 `ros_state` 字段，供前端区分「航点已派遣（DISPATCHED）」与「Nav2 真实执行状态（RUNNING/COMPLETED/FAILED）」。**注意：此话题反映 Nav2 FollowWaypoints action 的实际结果，比 GCS 侧的 waypoint_publisher 进程退出码更早暴露规划失败。**
...
- 默认栅格：**`workspace_nav/config/map_hk.yaml`**；备用的 **`map.yaml`** 可作模板对照  
