...
- GCS 发 **`/waypoint`** （见 **`gps_map_conversion.parse_waypoint_message`**：`waypoints`、`mission_id` 与可选 **`explicit_replan` / `from_start_mission` / `command` 等**，用于显式重申航迹）与可选 **`/color_code`**：
  - **`mission_bridge`**（`enable_mission_bridge:=true` 或单独 `mission_bridge.launch.py`）：常驻、mission hash、`waypoint_command_mode`（immediate/debounce/start_pulse）、**`mission_cancel_topic`**（Cancel Nav → 清缓冲并 cancel FollowWaypoints）、**`suppress_passive_waypoints_after_cancel`**（避免 cancel 后被被动 `/waypoint` 自动重跑）、写 **`target_buoy.json`**（**同色不写** + 可选 **`target_buoy_min_write_period_sec`**）、逐点 **FollowWaypoints**（`/odometry/filtered`）。**完整参数说明与常见问题**：[`docs/项目运行与联调.md`](../../docs/项目运行与联调.md) 中 **「三终端启动 → 地面站 `/waypoint`、`/color_code`」** 小节的 **`mission_bridge` 参数与行为**表格。
