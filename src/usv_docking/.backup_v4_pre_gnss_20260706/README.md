...
差速 / 双推进器 USV 的 **Tag 闭环两次到港** 精靠泊控制器（v4.0）。

> **任务编排（一键归港、Nav 预泊、Entry 验收）** 见 [`../dock_mission/README.md`](../dock_mission/README.md)。  
> 本包只管：**预泊点之后 → 搜 Tag → 对准 → 倒船 → 停船/充电确认**。
...
| **用什么导航** | Nav2 + RTK/map | AprilTag 视觉 + 里程计 |
| **谁启动它** | GCS 发 `/dock/home` | 收到 `/dock/start`（由 dock_mission 或 handoff 脚本发） |
...
/dock/home                         Nav2 手动到预泊点
    ...
    → Entry 验收                           → 本包接管
    → /dock/start ──────────────►    本包：WAIT_TAG → APPROACH_ENTRY → BACK_IN → STOP
```
...
/dock/start（且 Nav2 controller 已 deactivate）
  → PRECHECK → WAIT_TAG → (无 Tag) SEARCH_SPIN 自转搜 Tag
  → APPROACH_ENTRY（第一次到港，虚拟入口 depth=entry_standoff_m）
  → BACK_IN（第二次到港，真中心 depth→0）→ STOP
                                                      ↓
...
位姿语义详解：[`docs/归港对准与位姿说明.md`](docs/归港对准与位姿说明.md)

---

### ★ 第二优先级：预泊 / 搜 Tag（「到了预泊点但不动」）
...
### ★ 第三优先级：对准 / 通道门槛（「有 Tag 但不进 BACK_IN」）
...
### ★ 第四优先级：倒船入坞（「进了 BACK_IN 但撞/偏/停不住」）
...
| `success` / `needs_reapproach` / `abort_reason` / `error_code` | 结果（`dock_mission` 订阅） |
| `docking_time_sec` | 本次归港耗时 |
| `x_base`, `y_base`, `heading_error` | 控制位姿 |
...
全程 **Tag→base_link** 同一套 P 控制，只改深度目标；odom 仅用于搜 Tag 转角与传感器看门狗。
