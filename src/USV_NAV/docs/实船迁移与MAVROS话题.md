# 实船 MAVROS / Nav2 要点（ROS 2 + PX4）

> **本仓库（USV_NAV）仅维护实船。** 下文「仿真链」仅供与 **`wuxihik_navigation`** 对照，勿在本仓启动 Gazebo/`converter`。
...
Nav2 → /cmd_vel_nav → converter.py
     → /usv/thrusters/left|right/thrust → ros_gz → 推进器插件
```
...
短期联调可把 **Twist 转换为 MAVROS PositionTarget**；正式航行往往还要 ** LOS / MPC / Mixer**（见下文「架构建议」）。本仓库已提供：**`nav2_cmd_vel_to_mavros`**（`/cmd_vel_nav` → `/mavros/setpoint_raw/local`），以及 **`use_mavros_odometry`** 走 **`/mavros/local_position/odom`**，均属「减少重复造轮子」的过渡手段，不等于最终船控形态。
...
| **`setpoint_raw/local`** | **`mavros_msgs/PositionTarget`**；本项目 **`nav2_cmd_vel_to_mavros`** 默认写入此话题（velocity.x=线速度，velocity.y=偏航角速度）。联调：`ros2 topic echo /mavros/setpoint_raw/local`。 |
...
| 话题 | 含义 |
|------|------|
| **`rc/in`、`rc/out`** | 遥控输入链路监视 / **PWM 输出监视**（MAVROS 订阅 MAVLink 后发布）；联调 OFFBOARD 时可与 **`/mavros/setpoint_raw/local`** 对照（`ros2 topic echo`）。 |
