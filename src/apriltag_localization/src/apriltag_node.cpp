...
AprilTagLocalization::AprilTagLocalization(rclcpp::Node *node)
    : private_node_ptr_(node),
      logger_(private_node_ptr_->get_logger()),
      WORKSPACE_DIR(ament_index_cpp::get_package_prefix("apriltag_localization"))
{
  if (private_node_ptr_ == nullptr)
  {
    throw std::invalid_argument("AprilTagLocalization: node is null");
  }
  MLOGGER_INFO(
...
void AprilTagLocalization::main_loop()
{
  while (running_.load())
  {
    ...
    if (!cond_var_.wait_for(lock, std::chrono::milliseconds(2000),
                            [&] { return is_image_received_.load() || !running_.load(); }))
    {
      if (!running_.load())
      {
        break;
