"""Tag-based two-stage docking: virtual entry center → real dock center (v4.0)."""
...
class DockingController(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            f"usv_docking v4 two-stage ready entry_standoff="
            f"{self._entry_standoff_m}m rate={self._control_rate}Hz"
        )
    ...
    def _transition(self, new_state: DockState, reason: Optional[str] = None) -> None:
        ...
        if new_state == DockState.WAIT_CHARGE and old != DockState.WAIT_CHARGE:
            ...
            self._wait_charge_start_time = None
        if new_state in (DockState.IDLE, DockState.STOP):
            self._publish_zero_velocity()
    ...
    def _apply_limits(self, v_target: float, w_target: float) -> tuple[float, float]:
        dt = 1.0 / max(self._control_rate, 1.0)
        v = _clamp(
    ...
    def _control_timer_cb(self) -> None:
        ...
        if (
            ...
            and self._state not in (DockState.ABORT, DockState.STOP)
        ):
            self._publish_zero_velocity()
        ...
        if self._state == DockState.PRECHECK:
            ...
            if (
                ...
                self._transition(
                    DockState.APPROACH_ENTRY,
                    "tag found, first docking to virtual entry",
                )
            else:
                if self._resume_state_after_search is not None:
    ...
    def _publish_status(self) -> None:
        ...
        payload = {
            ...
            "x_base": round(float(self._x_base), 4),
            "y_base": round(float(self._y_base), 4),
